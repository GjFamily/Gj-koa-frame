...
var weixin = require('./../../helps/third/weixin');

let weixin_client = require('./../api/wx').client;
 ...
 */
module.exports.getSource = trade.getSource = function* ({ channel }) {
...
module.exports.query = function* ({ pay }) {
  let help = getHandle(pay.channel);
 ...
 * @param info
 */
module.exports.client = trade.client = function* ({ instance, pay, info }) {
  let source = CHANNEL_MAP[pay.channel];
...
module.exports.pay = function* ({ money, channel, module, module_id, pid, subject, body, kwargs }) {
  let pay = yield pay_model.make(money, channel, module, module_id, pid, subject, body, kwargs.client_ip);
  return pay;
};
/**
 * 预处理验证
...
module.exports.preprocess = function* ({ instance, kwargs }) {
  ...
  if (instance.pay_id || instance.pay_id != '') {
    let old_pay = yield pay_model.get(instance.pay_id);
    // 对于已有的支付进行处理
    if (old_pay) {
      if (old_pay.isPaying()) {
        // 对于还在处理中的结束订单
        let result = yield end(old_pay, old_pay.getResult());
        if (!result)
  ...
  if (channel == pay_model.CHANNEL.weixin_js || channel == pay_model.CHANNEL.weixin_qr || channel == pay_model.CHANNEL.weixin_wap || channel == pay_model.CHANNEL.weixin_app || channel == pay_model.CHANNEL.weixin_lite || channel == pay_model.CHANNEL.weixin_bar) {
...
module.exports.process = function* ({ instance, pay, kwargs }) {
  ...
  let notify_url = getNotifyUrl(pay.channel);
  let subject = pay.subject;
  let pay_id = pay.id;
  let body = pay.body;
  let money = pay.money;
  let pid = pay.pid;
  let channel = pay.channel;
  ...
  if (source == SOURCE.LOCAL) {
    ...
    // 条码支付
    let info = yield alipay_client.payTrade({ pay_id, auth_code: kwargs.auth_code, pid, money, subject, body, notify_url });
    yield pay.setInfo(info);
    // 循环查询直到得到结果
    return yield trade.client({ instance, pay, info });
  } else if (channel == pay_model.CHANNEL.weixin_bar) {
    // 条码支付
    let info = yield weixin_client.payTrade({ pay_id, auth_code: kwargs.auth_code, pid, money, subject, body, ip: kwargs.client_ip });
    yield pay.setInfo(info);
    // 循环查询直到得到结果
    return yield trade.client({ instance, pay, info });
  } else {
    ...
    if (channel == pay_model.CHANNEL.weixin_app) {
      info = yield weixin_client.appTrade({ pay_id, pid, money, subject, body, notify_url, ip: kwargs.client_ip });
    } else if (channel == pay_model.CHANNEL.weixin_js) {
      info = yield weixin_client.wapTrade({ pay_id, pid, money, subject, body, notify_url, ip: kwargs.client_ip, open_id: kwargs.open_id });
    } else if (channel == pay_model.CHANNEL.weixin_qr) {
      info = yield weixin_client.preTrade({ pay_id, pid, money, subject, body, notify_url, ip: kwargs.client_ip });
    } else if (channel == pay_model.CHANNEL.weixin_lite) {
      info = yield weixin_client.liteTrade({ pay_id, pid, money, subject, body, notify_url, ip: kwargs.client_ip, open_id: kwargs.open_id });
    } else if (channel == pay_model.CHANNEL.alipay_wap) {
      info = yield alipay_client.wapTrade({ pay_id, pid, money, subject, body, return_url: kwargs.return_url, notify_url });
    } else if (channel == pay_model.CHANNEL.alipay_app) {
      info = yield alipay_client.appTrade({ pay_id, pid, money, subject, body, notify_url });
    } else if (channel == pay_model.CHANNEL.alipay_qr) {
      info = yield alipay_client.preTrade({ pay_id, pid, money, subject, body, notify_url });
    }
    if (info.status == 'ok') {
...
function* end(pay, info) {
  let channel = pay.channel;
  let pay_id = pay.id;
  let transaction_no = pay.transaction_no;
  let pid = pay.pid;
  let result;
  // 根据渠道选择是撤销还是关闭操作
  ...
  } else if (channel == pay_model.CHANNEL.alipay_app || channel == pay_model.CHANNEL.alipay_pc || channel == pay_model.CHANNEL.alipay_wap) {
    result = yield alipay_client.closeTrade({ pay_id, transaction_no, pid })
  } else if (channel == pay_model.CHANNEL.weixin_bar) {
    result = yield alipay_client.cancelTrade({ pay_id, transaction_no, pid });
  } else if (channel == pay_model.CHANNEL.weixin_js || channel == pay_model.CHANNEL.weixin_app || channel == pay_model.CHANNEL.weixin_wap || channel == pay_model.CHANNEL.weixin_qr || channel == pay_model.CHANNEL.weixin_lite) {
