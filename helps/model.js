...
Executor.prototype._field = function(field){
  if(field instanceof Array){
    ...
    return field.join(",");
  }else if(field instanceof String && field){
    return field;
  }else{
    return '*';
  }
    ...
    if(this.is_new){
      for (let i in keys) {
        ...
        if (this.data[field] == undefined) {
          this.data[field] = schema.default[field] || '';
        }
      }
    }
  };
...
Model.prototype.record = function(data, is_new=true){
...
Model.prototype.delete = function(data){
  ...
  if(field){
    ...
    let query = {
      where: [[primary, "=", data.get(primary)]]
    };
    return this.deleteByQuery(query);
  }
};
...
Model.prototype.findByQuery = function(query){
  ...
  return executor.handle(query).then((result)=>{
    ...
    for(let i in result){
      result[i] = this.record(result[i], false);
    }
    return result;
  });
};
...
Model.prototype.insert = function(data, query={}){
  return this.insertBatch([data], query).then((result)=>{
    return result[0] || null;
  });
};
Model.prototype.insertBatch = function(list, query={}){
  let model = this;
  ...
  for(var index in list){
    var data = list[index];
    if(!(data instanceof this.Record)) data = this.record(data);
